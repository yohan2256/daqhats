...
중량 측정은 시작 전에 선택 채널 모두의 실제 원시 데이터 이력을 확인합니다.
`Preparing` 동안에는 타격하지 말고, `Capture now` 표시 후 타격하세요.
3초의 필터 초기화 데이터와 블록 전송 여유가 준비된 뒤 측정 시간이 시작됩니다.
최대 10초 동안 준비되지 않으면 수집/트리거 확인 메시지를 표시합니다.
측정 후 데이터를 더 기다려 분석 창을 이동시키거나 초기화 길이를 줄이지 않습니다.
WAV 녹음을 켠 경우 준비 구간도 파일 앞부분에 포함될 수 있으며,
평가값은 기존과 동일하게 최종 raw snapshot의 지정 측정 구간으로 계산합니다.
미보정 채널은 계속 거부합니다.

일반 음압레벨·실간 차음·외벽/창호 측정은 **Measurement modes** 메뉴에서 사용합니다.
절차와 지원 범위는 [ACOUSTIC_MODES.md](ACOUSTIC_MODES.md)를 참고하세요.
